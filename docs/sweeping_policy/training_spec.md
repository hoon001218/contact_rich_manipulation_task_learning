...
- Shelf frame: shelf root는 환경 로컬 기준 `(-0.7, 0, 0)`에 놓인다. 정렬 reward는 shelf의 기본 orientation에서 얻은 z축과 EEF의 y축을 비교한다.

점 `p_w`를 EEF 좌표로 바꾸는 계산은 다음과 같다.

```text
p_e     = transpose(R_we) * (p_w - p_eef_w)
p_eef_w = p_body_w + R_we * [0.13, 0, 0]
```
...
```text
x ~ Uniform(-0.78, -0.57) m
y ~ Uniform(-0.22,  0.22) m
z = 1.05 m
```

이고 yaw도 무작위다. 나머지 5개 물체는 서로 분리된 고정 (x,y) slot의 `z=1.80 m` 대기 선반에 놓인다. 물체별 width는 코드 순서대로 `(0.06, 0.06, 0.05, 0.09, 0.09, 0.06) m`이다.

Sweep 방향 `s`는 `-1` 또는 `+1`이며, 목표가 spawn 범위 안에 남도록 선택한다. 양쪽 모두 가능한 경우에만 50:50으로 고르고, 한쪽만 가능하면 가능한 쪽을 강제한다.

```text
sweep_direction_w = [0, 0.18 * s, 0]
target_goal_pos_w = target_reset_pos_w + sweep_direction_w
```

정책 시작 전 EEF는 target의 진행 반대편·위쪽으로 IK 배치된다.

```text
reaching_pos_w = target_reset_pos_w + [-0.02, -s * width, +0.09] + noise
noise[x], noise[y], noise[z] ~ Uniform(-0.01, 0.01) m
```
...
|---:|---|---:|---|
| `0:8` | `joint_pos` | 8 | 첫 8 joint의 `q - q_default`, rad. 즉 arm 6개와 articulation 순서상 뒤따르는 gripper joint 2개 |
| `8:14` | `joint_vel` | 6 | arm 6 joint의 `q_dot - q_dot_default`, rad/s. 기본 속도는 0 |
| `14:21` | `actions` | 7 | Action manager가 보관한 직전 policy action |
| `21:24` | `target_obs_state` | 3 | `transpose(R_we) * (target_pos_w - eef_pos_w)`, EEF frame, 각 성분에 `Uniform(-0.01, 0.01) m` noise |
| `24:25` | `target_obj_width` | 1 | 선택 물체 width에 `Uniform(-0.01, 0.01) m` noise |
| `25:32` | `ee_pose` | 7 | EEF의 robot-root 상대 position 3 + unique quaternion 4 |
| `32:35` | `goal_pos` | 3 | `transpose(R_we) * (goal_pos_w - eef_pos_w)`, EEF frame, noise 없음 |
...
```text
relative_position_eef = 0.5 * action[0:3]  [m]
relative_axis_angle   = 0.5 * action[3:6]  [rad]
```
...
```text
reward = -0.03 * action_rate
         -0.03 * joint_velocity
         -0.50 * shelf_collision
         -0.50 * object_collision
         +2.00 * orientation_alignment
         +6.00 * pushing_target
         +9.00 * homing_after_sweep
```
...
- `action_rate`: Isaac Lab 기본 term으로 `sum((action[t] - action[t-1])^2)`.
- `joint_vel`: arm 6축에 대해 `sum(q_dot[j]^2), j=1..6`.
...
```text
left   = clip(1 - (left_finger_z  - shelf_reference_z) / 0.02, 0, 1)
right  = clip(1 - (right_finger_z - shelf_reference_z) / 0.02, 0, 1)
wrist  = clip(1 - (wrist_z        - shelf_reference_z) / 0.08, 0, 1)
```

### 정렬, pushing, homing

정렬은 world에서 EEF y축과 shelf 기본 z축의 내적 `c`를 구해 다음처럼 계산한다.

```text
c = dot(eef_y_axis_w, shelf_z_axis_w)
alignment_reward = sign(c) * c^2
```

같은 방향이면 최대 +1, 반대면 최소 -1이다.

Pushing에서 접촉 유도점은 reset pose와 동일한 형태로 매 step 현재 target 위치에서 계산한다.

```text
contact_pos_w = target_pos_w + [-0.02, -s * width, +0.09]
```

`distance(EEF, contact) < 0.04 m`이면서 `abs(contact_y - wrist_y) < 0.04 m`이면 `contact_gate = 1`, 아니면 0이다. 목표 거리 `d = norm(goal_pos_w - target_pos_w)`와 target의 world y축 속도 `v_y`에 대해 다음과 같이 계산한다.

| 속도 조건 | `speed_reward` |
|---|---:|
| `abs(v_y) <= 0.05 m/s` | 0.0 |
| `0.05 < abs(v_y) < 0.1 m/s` | +0.5 |
| `abs(v_y) >= 0.1 m/s` | -0.5 |

| 목표 거리 조건 | `pushing_reward` |
|---|---|
| `d < 0.03 m` | `2 * exp(-5 * d)` |
| `d >= 0.03 m` | `contact_gate * (1 - d / 0.18 + speed_reward)` |

이다. 속도의 **방향은 검사하지 않고 절댓값만 검사**한다. 목표 반대 방향으로 움직여도 속도 구간만 맞으면 같은 shaping을 받을 수 있다.

Homing은 y, z 성분만 사용한 목표 거리와 첫 5개 arm joint의 기본 자세 오차로 계산한다.

```text
d_yz = norm((goal_pos_w - target_pos_w)[y, z])
joint_error = sum(abs(q[j] - q_default[j])), j=1..5

home_pose_reward = exp(-0.5 * joint_error)
goal_gate = (1 - tanh(100 * (d_yz - 0.03))) / 2
homing_reward = home_pose_reward * goal_gate
```

Target이 목표의 3 cm 안에 들어오면 gate가 켜지고, 이후 default pose로 돌아갈수록 보상이 커진다. wrist 3번 joint(6번째 arm joint)는 homing 오차에서 제외된다.

참고로 weight 0인 reaching term은 `exp(-10 * norm(contact_pos_w - eef_pos_w))`이다.
