"""UR5e/ROAS left-hand assembly and shelf-sweep asset paths."""
...
# Only these six independent hand joints are commanded by the policy. The other
# revolute joints retain the mimic relationships authored in the vendor URDF.
HAND_JOINT_NAMES = (
    "left_thumb_1_joint",
    "left_thumb_2_joint",
    "left_index_1_joint",
...
# One rigid link exists for every URDF mesh whose filename contains
# ``force_sensor``. Keeping this explicit makes sensor ordering deterministic.
...
EEF_BODY_NAME = "palm_force_sensor"
CONTACT_FORCE_THRESHOLD = 0.1

NUCLEUS_ROOT = "omniverse://192.168.0.13"
DEFAULT_UR5E_USD_PATH = (
    f"{NUCLEUS_ROOT}/NVIDIA/Assets/Isaac/5.0/"
...
DEFAULT_HAND_URDF_PATH = (
    REPOSITORY_ROOT
    / "assets/robots/Roas_provided_urdf/urdf/urdf_left_with_force_sensor.urdf"
)
...
def _force_sensor_links_from_urdf(source: Path) -> tuple[str, ...]:
    """Return links backed by a mesh whose filename contains ``force_sensor``."""
    links: list[str] = []
    for link in ElementTree.parse(source).findall("link"):
        meshes = link.findall("./visual/geometry/mesh") + link.findall("./collision/geometry/mesh")
        if any("force_sensor" in Path(mesh.attrib["filename"]).stem for mesh in meshes):
            links.append(link.attrib["name"])
    return tuple(links)


def validate_hand_urdf_assets(urdf_path: str) -> None:
    """Validate mesh files and the force-sensor link contract before conversion."""
    ...
    for mesh in ElementTree.parse(source).findall(".//mesh"):
        mesh_path = (source.parent / mesh.attrib["filename"]).resolve()
    ...
    if lfs_pointers:
        ...
        raise RuntimeError(
            f"Hand mesh files are Git LFS pointers, not mesh binaries "
            f"({len(lfs_pointers)} references). Examples: {preview}"
        )

    discovered = _force_sensor_links_from_urdf(source)
    if set(discovered) != set(FORCE_SENSOR_BODY_NAMES):
        missing_sensors = sorted(set(FORCE_SENSOR_BODY_NAMES) - set(discovered))
        unexpected_sensors = sorted(set(discovered) - set(FORCE_SENSOR_BODY_NAMES))
        raise RuntimeError(
            "The force-sensor links in the hand URDF do not match the environment configuration. "
            f"Missing: {missing_sensors}; unexpected: {unexpected_sensors}."
        )
...
def _find_named_prim(
    ...
    *,
    ...
    for candidate in candidates:
        for prim in prims:
            if prim.GetName() == candidate and (
                not rigid_body_only or prim.HasAPI(UsdPhysics.RigidBodyAPI)
            ):
                return prim
...
def _create_fixed_joint(
    ...
    parent_pos, parent_rot = _matrix_to_pose(
        joint_frame_world * _world_transform(parent_body).GetInverse()
    )
    child_pos, child_rot = _matrix_to_pose(
        joint_frame_world * _world_transform(child_body).GetInverse()
    )
...
@clone
def spawn_ur5e_roas_hand(
    prim_path: str,
    cfg: "Ur5eRoasHandSpawnerCfg",
    translation: tuple[float, float, float] | None = None,
    orientation: tuple[float, float, float, float] | None = None,
    **kwargs,
) -> Usd.Prim:
    """Spawn UR5e and the vendor left-hand URDF as one articulation."""
    del kwargs
    ...
    if not robot_root.IsValid() or not hand_root.IsValid():
        raise RuntimeError(
            "UR5e or ROAS hand did not compose into the stage. Check "
            "ROAS_SWEEPING_UR5E_USD_PATH and ROAS_SWEEPING_HAND_URDF_PATH."
        )
    ...
    hand_base = _find_named_prim(
        stage,
        hand_path,
        cfg.hand_base_candidates,
    ...
    _create_fixed_joint(
        stage,
        f"{prim_path}/UR5e_ROAS_hand_mount_joint",
        tool_body,
        hand_base,
        mount_world,
    )
...
@configclass
class Ur5eRoasHandSpawnerCfg(RigidObjectSpawnerCfg):
    """Spawner configuration for the assembled robot."""

    func: Callable = spawn_ur5e_roas_hand
    ...
    # This compensates the vendor left-hand base frame at the UR5e flange.
    mount_rotation_deg: tuple[float, float, float] = (0.0, 90.0, 0.0)


def make_ur5e_roas_hand_cfg() -> ArticulationCfg:
    """Create the assembled UR5e/ROAS-hand articulation."""
    spawn_cfg = Ur5eRoasHandSpawnerCfg(
    ...
    return ArticulationCfg(
        ...
        init_state=ArticulationCfg.InitialStateCfg(
            # Match sweeping_policy's assembled-robot root pose exactly.
            pos=(0.0, 0.0, 0.79505),
            rot=(0.0, 0.0, 0.0, 1.0),
            joint_pos={
...
def make_force_contact_sensor_cfg(body_name: str) -> ContactSensorCfg:
    """Create one contact sensor for one force-sensor mesh/link."""
    if body_name not in FORCE_SENSOR_BODY_NAMES:
        raise ValueError(f"Unknown force-sensor body: {body_name}")
    return ContactSensorCfg(
        prim_path=f"{{ENV_REGEX_NS}}/Robot/ROASLeftHand/{body_name}",
        update_period=0.0,
        history_length=1,
        track_pose=True,
